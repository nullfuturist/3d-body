...
// Torso
const torso = addObject([0, 0, 0], [0, 1, 0], 1.5, 0.5, 0.3, 0.4, 0.25, null, null);

// Right upper arm - attach to vertex 0 (right side) at top of torso
const rUpperArm = addObject([0, 0, 0], [0.8, -0.2, 0], 0.8, 0.15, 0.15, 0.12, 0.12, torso, {index: 0, atEnd: true});
// Right lower arm
const rLowerArm = addObject([0, 0, 0], [0.7, -0.3, 0], 0.7, 0.12, 0.12, 0.1, 0.1, rUpperArm, null);

// Left upper arm - attach to vertex 3 (left side) at top of torso
const lUpperArm = addObject([0, 0, 0], [-0.8, -0.2, 0], 0.8, 0.15, 0.15, 0.12, 0.12, torso, {index: 3, atEnd: true});
// Left lower arm
const lLowerArm = addObject([0, 0, 0], [-0.7, -0.3, 0], 0.7, 0.12, 0.12, 0.1, 0.1, lUpperArm, null);

// Right thigh - attach to vertex 0 (right side) at base of torso
const rThigh = addObject([0, 0, 0], [0.1, -1, 0], 0.9, 0.18, 0.18, 0.15, 0.15, torso, {index: 0, atEnd: false});
// Right lower leg
const rLowerLeg = addObject([0, 0, 0], [0, -1, 0.1], 0.9, 0.15, 0.15, 0.12, 0.12, rThigh, null);

// Left thigh - attach to vertex 3 (left side) at base of torso
const lThigh = addObject([0, 0, 0], [-0.1, -1, 0], 0.9, 0.18, 0.18, 0.15, 0.15, torso, {index: 3, atEnd: false});
// Left lower leg
...
function render() {
    ...
    const viewMatrix = createViewMatrix([camX, 2, camZ], [0, 0, 0], [0, 1, 0]);
